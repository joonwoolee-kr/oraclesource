...
-- 서브쿼리: 쿼리문 안에 또 다른 쿼리문(SELECT, INSERT, UPDATE, INSERT)이 포함
--SELECT 
--FROM 
...
-- 20번 부서에 근무하는 사원 중 전체 사원의 평균 급여보다 높은 급여를 받는 사원 조회 + 부서명
SELECT
	E.*, D.DNAME, D.LOC
FROM
	EMP e
...
-- 30번 부서의 최대 급여보다 적은 사원 조회하는 결과와 같아짐
-- 다중행 서브쿼리로 할 때
SELECT *
FROM EMP e 
...
-- 30번 부서의 최소 급여보다 많은 사원 조회하는 결과와 같아짐
-- 다중행 서브쿼리로 할 때
SELECT *
FROM EMP e 
WHERE E.SAL > ANY (SELECT E2.SAL FROM EMP e2 WHERE E2.DEPTNO = 30)
...
-- EMP_TEMP의 사원들 중에서 급여가 2500 이하인 사원들의 추가수당을 50으로 수정
UPDATE EMP_TEMP 
SET COMM = 50
WHERE SAL <= 2500;

-- DEPT 테이블의 DEPTNO = 40 부서의 DNAME, LOC를 추출해서 DEPT_TEMP2에 DNAME, LOC 수정
UPDATE
	DEPT_TEMP2
SET
	(DNAME,
...
-- UPDATE
-- EXAM_EMP에 속한 사원 중 50번 부서에서 근무하는 사원들의 평균 급여보다 많은 급여를 받고 있는 사원들을 70번 부서로 변경
--UPDATE
--	EXAM_EMP
--SET
--	DEPTNO = 70
...
-- DDL(데이터 정의어)
-- 데이터 베이스 데이터를 보관하고 관리하기 위해 제공되는 여러 객체의 생성/변경/삭제 관련 기능
-- CREATE(생성) / ALTER(생성된 객체 수정) / DROP(생성된 객체 삭제)
...
-- 		db 버전에 따라 한글 문자 하나 당 2byte 할당 or 3byte 할당
...
CREATE TABLE EMP_DDL(
	EMPNO NUMBER(4,0),
	ENAME VARCHAR2(10), -- 영어 10자까지 가능 / 한글 3자리까지 가능
	JOB VARCHAR2(9),
	MGR NUMBER(4,0),
	HIREDATE DATE,
	SAL NUMBER(7,2),
...
-- 2) 직접 인덱스 생성
--CREATE INDEX 인덱스명 ON 테이블명(열이름1 ASC OR DESC, 열이름 ASC OR DESC...)
...
-- 2. 보안성: 노출되면 안 되는 커럼을 제외하여 접근 허용

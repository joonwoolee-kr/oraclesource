...
-- PL/SQL: 데이터베이스 관련 특정 작업을 수행하는 명령어 / 선언문을 모아둔 블록
-- SQL + 프로그래밍
...
-- 커서(cursor): SELECT문 또는 DML과 같은 SQL 구문 실행했을 때 해당 SQL을 처리하는 정보를 저장한 메모리 공간
...
-- 1) DML 트리거: INSERT, UPDATE, DELETE와 같은 DML 명령어를 기점으로 동작
...
CREATE TABLE EMP_TRG_LOG(
	TABLENAME VARCHAR2(10), -- DML이 수행된 테이블 명
...
BEGIN
	IF INSERTING THEN
		...
		VALUES('EMP_TRG', 'INSERT', :NEW.EMPNO, SYS_CONTEXT('USERENV', 'SESSTION_USER'), SYSDATE);
	ELSIF UPDATING THEN
		INSERT INTO EMP_TRG_LOG
		VALUES('EMP_TRG', 'UPDATE', :OLD.EMPNO, SYS_CONTEXT('USERENV', 'SESSTION_USER'), SYSDATE);
	ELSIF DELETING THEN
		INSERT INTO EMP_TRG_LOG
		VALUES('EMP_TRG', 'DELETE', :OLD.EMPNO, SYS_CONTEXT('USERENV', 'SESSTION_USER'), SYSDATE);
	END IF;
END;
/
